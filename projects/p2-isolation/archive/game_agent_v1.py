...
def custom_score_last(game, player):  # 2x weight on ID_improved, no ex grid
    ...
    p1_empties = len(set(empty_spaces).intersection(p1_vicinity))
    p2_empties = len(set(empty_spaces).intersection(p2_vicinity))
    ...
    return float((2 * (own_moves - (1.5 * opp_moves))) + (p1_empties - p2_empties) + loc_bonus)

def custom_score_2(game, player):  # no center bonus
    """Calculates the heuristic value of a game state from the point of view
    of the given player. This is the best performing heuristic function for the
    project submission.
    ...
    # return weighted delta of available moves and empty space + location bonus
    return float((own_moves - opp_moves) + (p1_empties - p2_empties) + loc_bonus)


def custom_score_X(game, player): # no ex grid
    """Calculates the heuristic value of a game state from the point of view
    of the given player. This is the best performing heuristic function for the
    project submission.
...
def custom_score_3_last(game, player): # no center bonus, no ex grid
    """Calculates the heuristic value of a game state from the point of view
    of the given player.
    ...
    return float(p1_empties - p2_empties)
...
def custom_score(game, player):
    ...
    for move in own_moves:
        count = len(game.get_legal_moves(player))
        if count > own_count:
            own_count = count
    ...
    for move in opp_moves:
        count = len(game.get_legal_moves(game.get_opponent(player))) 
        if count < opp_count:
            opp_count = count
